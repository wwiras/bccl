...
class GameTheoryNode(BaseNode):
    ...
    def punish_peer(self, peer_id):
        ...
        if len(self.peers) > 4:  # Maintain minimum connectivity
            ...
            # If we don't have enough connections, just reduce priority but don't disconnect
            self.connection_priority[peer_id] = 0.1  # Very low priority
            #print(f"Node {self.id} could not punish {peer_id} (too few connections)")
